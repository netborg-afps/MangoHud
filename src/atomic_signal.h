...
//#define _atomic_signal_debug(x) SPDLOG_DEBUG(x)
  ...
  public:
    ...
    void wait() {
      ...
      while (true) {
        ...
        if (unlikely(!res))
          SPDLOG_ERROR(str::format("WaitOnAddress failed. LastError: ", GetLastError()));
