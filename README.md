# Latency measurements branch

Frame-latencies are an important part of the gamer's quest to input-lag. This branch now implements the functionality to measure these (Vulkan-only for now).
...
`gpu_stats=0` was selected in this example as recent Nvidia drivers (545) have performance issues with querying GPU statistics.

The progress of this branch is noted in the PROGRESS file.
